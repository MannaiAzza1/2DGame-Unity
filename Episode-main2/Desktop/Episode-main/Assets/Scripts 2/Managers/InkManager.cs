...
public class InkManager : MonoBehaviour
{
    ...
    private void Bindfunctions(Story story)
    {
       story.BindExternalFunction("change_background", (int index) =>
        ...
        story.BindExternalFunction("play_voiceline", (int index) =>
        {
            PlayVoiceline(index);
        });
        story.BindExternalFunction("finished", (bool victory) =>
        {
            Finished(victory);
        });
    }
    ...
    private void Finished(bool victory)
    {
    }
    private void PlayVoiceline(int index)
    {
        SoundpManager.instance.PlaySound(index);
    }
    ...
    void RemoveChildren()
    {
        ...
        {
            if(canvas.transform.GetChild(i).name.ToLower().Contains("black")) continue;
